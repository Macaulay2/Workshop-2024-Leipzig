...
spohnMatrices (Ring, List) := List => (PR, X) -> (
    ...
    d := format X_0;
    J := indexset X_0;
    apply(n, i -> matrix apply(d_i, k -> {sum(select(J, j -> j_i==k), j -> (p_j)_PR),
                                          sum(select(J, j -> j_i==k), j -> (X_i)#j * (p_j)_PR) })))
...
konstanzMatrix (Ring, List) := Matrix => opts -> (PR, X) -> (
    ...
    P := vector(apply(J, j -> (p_j)_konstanzRing));
    fold((M0, M1) -> M0 || M1, 

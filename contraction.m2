...
contractAgainstBasisVector = (w,b) -> ( --b is the index of e_b !!!
    if w == 0 then return 0_(ring w);
    t := apply(listForm w, (exps,coef) -> (
        if exps#b == 0 then return 0_(ring w);
        sign := (-1)^(sum(b,i->exps#i));
        return sign * coef * expsToMon(replace(b,0,exps), ring w);
    ));
...
contractionMatrix = (i,j,a) -> (
    if i > numgens ring a or j < 0 then error "Exterior power is zero!";
    kk := baseRing ring a;
    Bi := listOfMonomials(i, ring a);
    Bj := listOfMonomials(j, ring a);
    if a == 0 then return map(kk^(#Bi),kk^(#Bi),0);
    assert(i-j == (degree a)_0);
